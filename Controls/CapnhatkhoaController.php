<?php
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') { 
    ...
    $user = new NguoiDung();
    $user->setMaSo(trim($makhoa));
    $user->setMatKhau(md5('123456'));
    $user->setMaQuyen('KH');
    $user->themNguoiDung();
        if ($khoa->isExits("Khoa","ND_ID='".$khoa->getMaSo()."'") <= 0  ){
            if ($khoa->themKhoa() > 0){
                echo "<script language=javascript> alert('Đã thêm thành công!'); </script>";
                 require('Views/CapNhatKhoa.php');
            }
            else{
            ...
            echo "<script language=javascript> alert('Mã khoa đã tồn tại, vui lòng nhập mã khác!'); </script>";
            require('Views/CapNhatKhoa.php');
        }
    ...
    if (isset($_POST['btnxoa'])){
        ...
        if ($khoa->isExits("BoMon","ND_ID='".$khoa->getMaSo()."'") <= 0  ){
               if ($khoa->xoaKhoa() > 0){
                $user = new NguoiDung();
                $user->setMaSo(trim($makhoa));
                $user->xoaNguoiDung();
            ...
            echo "<script language=javascript> alert('Không thể xóa khoa này do tồn tại bộ môn thuộc khoa này. Để xóa xin vui lòng xóa hết các bộ môn thuộc khoa này!'); </script>";
            require('Views/CapNhatKhoa.php');
        }
  ...
  else include_once('Views/CapNhatKhoa.php');
?>
