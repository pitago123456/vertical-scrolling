...
function getcongtrinh(id,name,tenct,khoa,bm,tchi,nxb,ncb,start) //ham lay thong tin ten cua can bo bang ma so hoac lay ten
    {     
    send='start='+start; 
    if(name !='')  send += '&name='+name;   
    if(id!='')  send += '&id='+id; 
    if(tenct!='') send += '&tenct='+tenct;
    if(khoa!='')send+='&khoa='+khoa; 
    if(bm!='')  send+='&bm='+bm;
    if(tchi!='')  send+='&tchi='+tchi;
    if(nxb!='')  send+='&nxb='+nxb;
    if(ncb!='')  send+='&ncb='+ncb;
      ...
      if (xmlhttp.readyState==4 && xmlhttp.status==200)
        {
         ...
         if(tenct!='') $('.line_c,.line_l').highlight(tenct); 
         if(tchi!='') $('.line_c,.line_l').highlight(tchi);
         if(nxb!='')  $('.line_c,.line_l').highlight(nxb);
         
                        
        }
      }
     xmlhttp.open("GET","Controls/AjaxTimcongtrinh.php?"+send+"&gr=QL&q="+Math.random(),true);
     xmlhttp.send();
