...
function getnghiencuu(id,name,tennc,khoa,bm,cdt,nbd,nkt,start) //ham lay thong tin ten cua can bo bang ma so hoac lay ten
    {
     
    send='start='+start; 
    if(name !='')  send += '&name='+name;   
    if(id!='')  send += '&id='+id;
    if(tennc!='')  send += '&tennc='+tennc; 
    if(khoa!='')send+='&khoa='+khoa; 
    if(bm!='')  send+='&bm='+bm;
    if(cdt!='')  send+='&cdt='+cdt;
    if(nbd!='')  send+='&nbd='+nbd;
    if(nkt!='')  send+='&nkt='+nkt;
    if(start!='')  send+='&start='+start;
      ...
      if (xmlhttp.readyState==4 && xmlhttp.status==200)
        {
         ...
         if(tennc!='') $('.line_c,.line_l').highlight(tennc);
         xmlhttp = null;
                         
        }
      }
     xmlhttp.open("GET","Controls/AjaxTimnghiencuu.php?"+send+"&gr=QL&q="+Math.random(),true);
     xmlhttp.send();
